...
export function InvoiceDetails({ invoice }: InvoiceDetailsProps) {
  // Add console logs for debugging payment and transaction data
  console.log('Full Invoice Data:', JSON.stringify(invoice, null, 2));
  if (invoice.payments) {
    console.log('Payments Data:', JSON.stringify(invoice.payments, null, 2));
    invoice.payments.forEach((payment, index) => {
      console.log(`Payment ${index + 1} Details:`, {
        id: payment.id,
        amount: payment.amount,
        method: payment.method,
        status: payment.status,
        transactions: payment.transactions,
        receipt_number: payment.transactions?.[0]?.receipt_number
      });
    });
  }
    ...
    <Card className="p-8 max-w-4xl mx-auto bg-white shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-8 mb-8">
        <div>
          ...
          <p className="font-medium text-gray-900">{invoice.organization?.name || 'Organization'}</p>
        </div>
        <div>
